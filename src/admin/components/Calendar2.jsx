...
export default function Calendar({ label }) {
  ...
  const conferences = ["All", "Conference A", "Conference B", "Conference C"];
  const [selectedConference, setSelectedConference] = useState("All");
  const [anchorElConference, setAnchorElConference] = useState(null);
  ...
  const handleOpenConferenceMenu = (event) => {
    setAnchorElConference(event.currentTarget);
  };

  const handleCloseConferenceMenu = () => {
    setAnchorElConference(null);
  };

  const handleSelectConference = (conference) => {
    setSelectedConference(conference);
    handleCloseConferenceMenu();
  };
  ...
  const generateTimeSlots = (start = "10:00", end = "15:00") => {
    const timeSlots = [];
  ...
  const filteredBookings = bookings.filter(
    (booking) =>
      ...
      (selectedConference === "All" || booking.conference === selectedConference)
  );
    ...
    <Box p={isMobile ? 2 : 4} position="relative">
      ...
      >
        ...
        <Box display="flex" alignItems="center" flexDirection={isMobile ? "column" : "row"}>
          <Button
            variant="contained"
            onClick={handleOpenConferenceMenu}
            sx={{
              mr: isMobile ? 0 : 2,
              mb: isMobile ? 1 : 0,
          ...
          <Menu
            anchorEl={anchorElConference}
            open={Boolean(anchorElConference)}
            onClose={handleCloseConferenceMenu}
          >
